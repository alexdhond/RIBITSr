...
#' Get CSV report info
#'
#' Returns metadata about a specific CSV report type.
#'
#' @param report_type Character. The report type.
#' @return A list with report metadata.
#' @export
#' @examples
#' rb_csv_info("banks_sites")
#' rb_csv_info("credit_classification")
rb_csv_info <- function(report_type) {
  if (!report_type %in% names(CSV_REPORT_REGISTRY)) {
    cli::cli_abort("Unknown report type: {report_type}")
  }
  CSV_REPORT_REGISTRY[[report_type]]
}


#' List CSV reports by structure type
#'
#' Shows which CSV reports are summary tables (safe to merge) vs
#' transaction tables (should be kept separate).
#'
#' @return A tibble with report types and their merge safety status.
#' @export
#' @examples
#' rb_csv_structure()
rb_csv_structure <- function() {
  ...
  for (rt in names(CSV_REPORT_REGISTRY)) {
    info <- CSV_REPORT_REGISTRY[[rt]]
  ...
  for (rt in names(CSV_REPORT_REGISTRY)) {
    info <- CSV_REPORT_REGISTRY[[rt]]
    if (!isTRUE(info$merge_safe)) {
      avg <- if (!is.null(info$avg_rows_per_entity)) {
        paste0(" (~", info$avg_rows_per_entity, " rows/entity)")
      } else ""
      cli::cli_alert_warning("{.strong {info$name}} ({rt}){avg}")
      cli::cli_text("   Grain: {info$grain} | ID: {info$id_col}")
    }
  }
  
  cli::cli_h2("Recommended Usage")
  cli::cli_text("
For harmonized bank data:
  {.code result <- rb_banks(state = 'CA')}
  {.code result$banks}      # Summary data merged from API + EPA + CSV
  {.code result$ledger}     # Transaction data (separate table)

For detailed credit data:

  {.code credits <- rb_get_report_data('credit_classification')}
  {.code credits |> filter(bank_name == 'My Bank')}
")
